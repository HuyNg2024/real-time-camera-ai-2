...
from ultralytics.utils.postgres import insert_results
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for batch in self.dataset:
                ...
                frame_id = getattr(self, "_postgres_frame_id", 0)
                every_n_frames = max(int(os.getenv("YOLO_POSTGRES_EVERY_N_FRAMES", "10")), 1)
                if frame_id % every_n_frames == 0:
                    ...
                    if inserted:
                        LOGGER.info(f"[Postgres] inserted {inserted} detections at frame {frame_id}")
                self._postgres_frame_id = frame_id + 1
